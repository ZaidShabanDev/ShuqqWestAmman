...
const LocationInput: FC<Props> = ({ onChange, className, defaultValue = 'United States', imputName = 'location' }) => {
  ...
  const renderSearchValues = ({ heading, items }: { heading: string; items: string[] }) => {
    return (
      <>
        <p className="block text-base font-semibold">{heading || t('Destinations')}</p>
        <div className="mt-3">
          {items.map((item) => {
            return (
              <div
  ...
  return (
    <div className={clsx(className)} ref={containerRef}>
      <h3 className="text-xl font-semibold sm:text-2xl">{t('Where to find?')}</h3>
      ...
      <div className="mt-7">
        {value
          ? // if input value is not empty, show suggestions based on input
            renderSearchValues({
              heading: t('Locations'),
              items: ['Afghanistan', 'Albania', 'Algeria', 'American Samoa', 'Andorra'],
            })
